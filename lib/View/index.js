...
View.prototype = {
   ...
   * @param  {Object} data      Data to send to template, may be res.locals context
   * @return {String}           compiled template html
   */
  renderTemplate(name, themeName, data) {
    const view = this;
    let theme = null;
    let templateThemeName;

    if (themeName) {
      if (typeof themeName == 'string') {
        theme = view.themes[themeName];
      } else if (typeof themeName == 'object' && themeName.name){
        // support for receive theme in themeName arg:
        theme = view.themes[ themeName.name ];
        themeName = themeName.name;
      }

      // unique name for current theme template
      templateThemeName = themeName + '/' + name;
    ...
    let template = view.getAndCompileTemplateToRenderSync(theme, templateThemeName, data, name);
    ...
    if (!template) {
      ...
      if (data && data.req) {
        if (data.req.originalUrl) {
          url = data.req.originalUrl;
        } else {
          url = data.req.url;
        }
      }
