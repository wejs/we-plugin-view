...
View.prototype = {
  ...
  renderLayout(req, res, data) {
    ...
    if (view.loadFromCache() && view.layoutCache[layoutThemeName]) {
  ...
  /**
   * render one template, first check if the template exists in theme if now fallback to plugin template
   *
  ...
  renderTemplate(name, themeName, data) {
    ...
    if (view.loadFromCache() && view.templateCache[templateThemeName]) {
      // theme template
    ...
    we.utils.async.series([
      function loadTemplates(done) {
        ...
        for (let templateName in we.view.configuration.templates) {
          let tpl = we.view.configuration.templates[templateName];

          if (!we.config.cacheThemeTemplates) {
            let isThemeTemplate = false;
            for (let i = 0; i < themeNames.length; i++) {
              if (templateName.indexOf(themeNames[i]) === 0) {
                // theme template, skip
            ...
            if (isThemeTemplate) continue;
          }

          // load the template
          cache.templates[templateName] = fs.readFileSync(tpl, 'utf8');
        }

        done();
      },
      function loadAll (done) {
        // optional support for cache theme templates:
        if (!we.config.cacheThemeTemplates) {
          // only project theme:
          return done();
        } else {
          // all templates:
          we.utils.async.each(themeNames, (themeName, next)=> {
            view.cacheAllThemeTemplates(themeName, themes, cache, next);
          }, done);
        }
      }
